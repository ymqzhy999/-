...
def table():
    data = pd.read_csv("../data/data.csv").head(20)
    headers = ["title", "comment_count", "averge_price", "type", "area", "recommend"]
    rows = data[headers].values

    table = (
        Table()
            .add(headers=headers, rows=rows)
            .set_global_opts(title_opts=ComponentTitleOpts(title="美食排行榜前20"))
    )

    table.render("templates/美食排行榜前20.html")
...
def funnel():
    ...
    c = (
        Funnel()
            ...
            .set_global_opts(title_opts=opts.TitleOpts(title="美食店铺评论人数top10", pos_bottom="2%", pos_left="37%"))
...
def wordcloud():
    ...
    c = (
        WordCloud()
            ...
            .set_global_opts(title_opts=opts.TitleOpts(title="zhengzhou"))
