...
function show_data(page_data, page_no, page_list) {
    ...
    for (var i = 0; i < page_data.length; i++) {
        ...
        list_data = list_data + '<tr>' +
            ...
            '<td>' + item[6] + '</td>' +
            '<td class="td-manage">' +
            ' <a title="编辑"  onclick="x_details_edit(\'编辑\',' + item[0] + ',\'' + item[1] + '\',\'' + item[2] + '\',\'' + item[3] + '\',' + item[4] + ',' + item[5] + ',' + item[6] + ')" href="javascript:;">' +
    ...
    if (page_no == 1) {
        ...
        page_str = '<span class="prev" onclick="get_details_data(' + (page_no - 1) + ')">&lt;&lt;</span>';
    }
    for (var i = 0; i < page_list.length; i++) {
        ...
        if (item == page_no) {
            ...
            page_str = page_str + '<span class="num" onclick="get_details_data(' + item + ')">' + item + '</span>'
        }
    }
    if (page_no != max_page) {
        page_str = page_str + ' <span class="next" onclick="get_details_data(' + (page_no + 1) + ')">&gt;&gt;</span>'
    }

    $("#details_data").html(list_data);
    $("#page_list").html(page_str);

}


function x_details_edit(title, a, b, c, d, e, f, g) {
    let w = 600;
    let he = 550;
    $('#id').val(a);
    $('#update_time').val(b);
